<template>
  <div class="relative">
    <button type="button" class="flex items-center gap-1 text-xs text-text-tertiary transition-colors hover:text-text-secondary"
            aria-label="切换住所" :aria-expanded="open"
            @click="open = !open">
      <!-- 触发器内容可替换（首页：日期 · 住所名） -->
      <slot>{{ auth.currentHousehold?.name ?? '…' }}</slot>
      <ChevronDown :size="14" class="transition-transform" :class="{ 'rotate-180': open }" aria-hidden="true" />
    </button>
    ...
    <div v-if="open"
         class="absolute left-0 top-8 z-30 w-56 rounded-xl border border-border bg-neutral-surface p-1.5 shadow-level-2">
