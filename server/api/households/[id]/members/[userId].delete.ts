...
export default defineEventHandler(async (event) => {
  ...
  if (role !== 'owner') throw createError({ statusCode: 403, statusMessage: '仅住所创建者可移除成员' })
  ...
  if (!target.length) throw createError({ statusCode: 404, statusMessage: '该用户不是住所成员' })
  if (target[0].role === 'owner') throw createError({ statusCode: 403, statusMessage: '不能移除住所创建者' })
