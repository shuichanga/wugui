<template>
  <div class="min-h-dvh lg:h-dvh lg:overflow-hidden grid lg:grid-cols-3">
    <!-- Left: Character Scene (full on desktop, scaled on mobile) -->
    <section class="relative overflow-hidden bg-gradient-to-br from-gray-400 via-gray-500 to-gray-600 lg:col-span-2">
      <!-- Mobile Header -->
      <header class="relative z-20 flex lg:hidden items-center gap-2 p-4 text-white">
        <Package :size="24" aria-hidden="true" />
        <span class="text-lg font-semibold">物归</span>
      </header>

      <!-- Desktop Brand -->
      <div class="relative z-20 hidden lg:block p-12">
      ...
      <!-- Character Scene -->
      <div class="absolute inset-x-0 bottom-0 flex items-end justify-center pb-12 lg:inset-0 lg:items-center lg:pb-0">
        <div class="scene-wrapper">
          <div ref="sceneRef" class="scene-inner">
            ...
            <div ref="purpleCharacter" class="character char-purple">
              <div ref="purpleEyes" class="eye-row char-purple-eyes">
                <div ref="purpleEye1" class="eye-ball eye-purple">
                  <div ref="purplePupil1" class="pupil pupil-purple"></div>
                </div>
                <div ref="purpleEye2" class="eye-ball eye-purple">
                  <div ref="purplePupil2" class="pupil pupil-purple"></div>
                </div>
              </div>
            </div>
            <!-- Black Character -->
            <div ref="blackCharacter" class="character char-black">
              <div ref="blackEyes" class="eye-row char-black-eyes">
                <div ref="blackEye1" class="eye-ball eye-black">
                  <div ref="blackPupil1" class="pupil pupil-black"></div>
                </div>
                <div ref="blackEye2" class="eye-ball eye-black">
                  <div ref="blackPupil2" class="pupil pupil-black"></div>
                </div>
              </div>
            </div>
    ...
    <!-- Right: Login Form -->
    <main class="flex flex-col justify-center bg-neutral-surface px-6 py-8 lg:col-span-1 lg:px-10">
      ...
      <form class="mt-6 flex flex-col gap-4" @submit.prevent="onSubmit">
        ...
        <section>
          ...
          <input id="login-email" ref="emailRef" v-model="email" type="email" class="form-input" autocomplete="email" placeholder="请输入邮箱" required @focus="onEmailFocus" @blur="onEmailBlur" />
        </section>
        ...
        <section>
          ...
          <div class="relative">
            <input id="login-password" ref="passwordRef" v-model="password" :type="showPassword ? 'text' : 'password'" class="form-input pr-10" autocomplete="current-password" placeholder="请输入密码" required @input="onPasswordInput" />
            <button type="button" class="absolute right-3 top-1/2 -translate-y-1/2 text-text-tertiary transition-colors hover:text-text-primary" aria-label="切换密码可见" @click="togglePassword">
...
import { Eye, EyeOff, ArrowRight, Package } from 'lucide-vue-next'
...
const CHAR = {
  purple: { left: 60, width: 160, height: 340, eyeLeft: 40, eyeTop: 34, eyeSize: 16, pupilSize: 6, gap: 18, maxDist: 4.5 },
  black: { left: 210, width: 105, height: 265, eyeLeft: 23, eyeTop: 28, eyeSize: 14, pupilSize: 5, gap: 16, maxDist: 3.5 },
  orange: { left: 0, width: 210, height: 170, eyeLeft: 73, eyeTop: 78, pupilSize: 11, gap: 25, maxDist: 5 },
  yellow: { left: 270, width: 125, height: 200, eyeLeft: 46, eyeTop: 36, pupilSize: 11, gap: 25, mouthLeft: 36, mouthTop: 76, mouthWidth: 48, maxDist: 5 },
}
...
function onEmailFocus() { state.isTyping = true; triggerLook(); render() }
function onEmailBlur() { state.isTyping = false; render() }
function onPasswordInput() {
  state.passwordLength = password.value.length
  state.showPassword = showPassword.value
  schedulePeek()
  render()
}
...
@media (max-width: 1023px) {
  .scene-wrapper { transform: scale(0.65); transform-origin: center center; }
}
</style>
